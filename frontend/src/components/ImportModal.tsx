...
export default function ImportModal({ open, onClose }: ImportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl shadow-cyan-950/20">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-cyan-400">Beds24</p>
            <h2 className="mt-1 text-2xl font-semibold text-white">Import bookings</h2>
          </div>
        ...
        {loading ? <p className="mt-6 text-sm text-slate-400">Loading bookings...</p> : null}
        ...
        <div className="mt-6 max-h-[60vh] space-y-3 overflow-y-auto pr-1">
          {bookings.map((booking) => (
            <div key={booking.booking_id} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-base font-semibold text-white">{booking.guest_name}</p>
                  <p className="mt-1 text-sm text-slate-400">Booking ID {booking.booking_id}</p>
                  <p className="mt-1 text-sm text-slate-500">{booking.status || 'Unknown status'}</p>
                </div>
