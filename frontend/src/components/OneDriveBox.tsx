...
export default function OneDriveBox({ tenantId }: OneDriveBoxProps) {
  ...
  const handleChooseFolder = async () => {
    ...
    try {
      ...
      setRootHandle(selectedRoot)
      await resolveTenantFiles(selectedRoot)
    } catch (err) {
      if (err instanceof DOMException && err.name === 'AbortError') return
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          disabled={loading || unsupported || !tenantId || !tenantBookingId}
          className="rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-50"
        >
          Choose folder
        </button>
      </div>
