...
def get_beds24_availability_summary(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if row is None:
        return Beds24AvailabilitySummaryRead(summary_text="Availability has not been fetched yet.", refreshed_at=None)
    return Beds24AvailabilitySummaryRead(summary_text=row.summary_text, refreshed_at=row.refreshed_at)
