...
def test_send_to_beds24_forwards_empty_flag_text(client, auth_headers, monkeypatch):
    captured = {}
    ...
    response = client.post(
        "/api/quotation/12345/send-to-beds24",
        json={"all_original_invoice_item_ids": [], "invoice_items": [], "flag_text": ""},
        headers=auth_headers,
    )

    assert response.status_code == 200
    # A cleared Flag is forwarded as "" so the CRM clears the Beds24 flag.
    assert captured["payload"]["flag_text"] == ""
